...
var (
	...
	IntegrationPipelineRunAttemptTotal = prometheus.NewCounter(
		prometheus.CounterOpts{
...
func RegisterRunningSnapshot(startTime metav1.Time, inProgressTime *metav1.Time) {
	SnapshotRunningSeconds.Observe(inProgressTime.Sub(startTime.Time).Seconds())
}

func RegisterIntegrationResponse(buildPipelineFinishTime metav1.Time, inProgressTime *metav1.Time) {
	SnapshotRunningSeconds.Observe(inProgressTime.Sub(buildPipelineFinishTime.Time).Seconds())
}
...
func RegisterNewIntegrationPipelineRun() {
	IntegrationPipelineRunAttemptTotal.Inc()
}
...
func init() {
	metrics.Registry.MustRegister(
		...
		IntegrationPipelineRunAttemptTotal,
